...
object Basics {
  ...
  val allBooleans: Set[Boolean] = Set(false, true)
  ...
  val allOptionBooleans: Set[Option[Boolean]] = Set(None, Some(false), Some(true))

  // Exercise. List all values of the type `Either[Unit, Boolean]`:
  val allEitherUnitBooleans: Set[Either[Unit, Boolean]] = Set(Left(), Right(false), Right(true))
  ...
  val allTupleBooleanBooleans: Set[(Boolean, Boolean)] = Set((false, false), (false, true), (true, false), (true, true))
